...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        member.ban({reason: reason, days: 7}).then(() => {
            message.guild.members.unban(member.id, `SOFTBAN`)
        })

        const embed = new MessageEmbed()
        .setAuthor(`Softbanned Member!`, client.user.displayAvatarURL())
        .setDescription(`${client.custom_emojis.tick} • Succesfully softbanned <@${member.id}> (\`${member.id}\`) from ${message.guild.name}`)
