...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        .setTitle(`Counting system! [1]`, client.user.displayAvatarURL())
        ...
        .setTitle(`Counting system! [2]`, client.user.displayAvatarURL())
        .setDescription(`What should the counting message channel be?`)
        .setColor(client.colors.pink)
        .setFooter(`You can say "cancel" at any time to cancel the process`)
        ...
        collector.on('end', async (collected, reason) => {
            ...
            if(reason === "2") {
                await guilds.findOneAndUpdate({guildId: message.guild.id}, {
                    counting: true,
                    counting_channel: finalData.channel,
                })
                return message.channel.send({content: `${client.custom_emojis.tick} • Counting data has now been setup!`})
            }
        })
    }
}