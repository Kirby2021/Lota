...
module.exports = {
    ...
    run: async(client, message, args, {prefix}) => {
        ...
        .setTitle(`Suggestions! [1]`, client.user.displayAvatarURL())
        ...
        .setTitle(`Suggestions! [2]`, client.user.displayAvatarURL())
        .setDescription(`What should be the suggestions channel?`)
        .setColor(client.colors.pink)
        .setFooter(`You can say "cancel" at any time to cancel the process`)
        ...
        collector.on('end', async (collected, reason) => {
            ...
            if(reason === "4") {
                ...
                return message.channel.send({content: `${client.custom_emojis.tick} • Suggestions have now been disabled!`})
                
            }
            if(reason === "2") {
                await guilds.findOneAndUpdate({guildId: message.guild.id}, {
                    suggestions: true,
                    suggestions_channel: finalData.channel,
                })
                return message.channel.send({content: `${client.custom_emojis.tick} • Suggestions data has now been setup!`})
            }
        })
    }
}